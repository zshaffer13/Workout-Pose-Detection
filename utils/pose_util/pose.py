# -*- coding: utf-8 -*-
"""
Created on Thu Mar 27 23:52:56 2025
...
from utils.ops_utils import Operation
from utils.timer_utils import Timer
from utils.pose_drawing_util import Draw
from utils.pose_util.consts import POSE, PRESENCE_THRESHOLD, VISIBILITY_THRESHOLD
...
class Pose():
    ...
    def get_keypoints(self, image, pose_result):
        ...
        for idx, landmark in enumerate(pose_result.pose_landmarks.landmark):
            if ((landmark.HasField('visibility') and landmark.visibility < VISIBILITY_THRESHOLD) or
                (landmark.HasField('presence') and landmark.presense < PRESENCE_THRESHOLD)):
                continue
    ...
    def get_available_point(self,points):
        ...
        First index is 1st priority and so on.
        """
    ...
    def one_line_angle(self, str_point1, str_point2):
        ...
        coord1 = self.get_point(str_point1)
        coord2 = self.get_point(str_point2)
        return self.operation.angle_of_single_line(coord1,coord2)
    ...
    def predict_pose(self):
        ...
        #Calculate angle between the lines of shoulder-elbow and elbow-wrist
        ...
        #Calculate angle between the lines of shoulder-hip and hip-ankle
        if self.is_point_in_keypoints("left_shoulder") and \
            self.is_point_in_keypoints("left_hip") and \
                self.is_point_in_keypoints("left_ankle"):
                    ang1 = self.two_line_angle("left_shoulder", "left_hip", "left_ankle")
        elif self.is_point_in_keypoints("right_shoulder") and \
            self.is_point_in_keypoints("right_hip") and \
                self.is_point_in_keypoints("right_ankle"):
                    ang1 = self.two_line_angle("right_shoulder", "right_hip", "right_ankle")
        else:
            pass
        
        #Calculate angle of shoulder-ankle or hip-ankle
        ...
        if left_hip_knee or right_hip_knee:
            ...
            ang6 = self.one_line_angle(knee, hip)
        else:
            pass
        ...
        if ang3 is not None and ((0 <= ang3 <= 50) or (130 <= ang3 <= 180)):
            if (ang1 is not None or ang2 is not None) and ang4 is not None:
                if (160 <= ang2 <= 180) or (0 <= ang2 <= 20):
                    ...
                    self.ang4_trackekr.append(ang4)
        ...
        if self.pushup_counter >= 24 and len(self.ang1_tracker) == 24 and len(self.ang4_tracker) == 24:
            ...
            ang1_diff_mean = (ang1_diff2 + ang1_diff1) /2
        ...
        if diff_head_hand_y is not None and ang3 is not None:
            if diff_head_hand_y > 0 and 80< ang3 <=180:
        ...
        if len(self.ang1_tracker) == 24:
            del self.ang1_tracker[0]
        if len(self.ang4_trackekr) == 24:
            del self.ang4_trackekr[0]
    ...
    def estimate(self) -> None:
        ...
        while self.video_reader.is_opened():
            ...
            #Improve performance by marking image as not writable to
...
class Pushup(Pose):
    ...
    def measure(self) -> None:
        """Measure pushups (base func)"""
        if self.video_reader.is_opened() is False:
            print("Error File Not Found")
            
        out = cv2.VideoWriter("pushup_test_output.avi", self.fourcc, self.video_fps,(self.width, self.height))
...
class Plank(Pose):
    ...
    def _draw(self, image):
        """Draw lines between shoulder, wrist and foot"""
        left_shoulder_elbow_foot = self.is_point_in_keypoints("left_shoulder") and \
                                    self.is_point_in_keypoints("left_elbow") and \
                                    self.is_point_in_keypoints("left_foot_index")
                                    
        right_shoulder_elbow_foot = self.is_point_in_keypoints("right_shoulder") and \
                                    self.is_point_in_keypoints("right_elbow") and \
                                    self.is_point_in_keypoints("right_foot_index")  
        if left_shoulder_elbow_foot:
            ...
            image = self.draw.draw_line(image, self.get_point("left_foot_index"), self.get_point("left_shoulder"))
        elif right_shoulder_elbow_foot:
    ...
    def pose_algorithm(self):
        ...
        #Calculate angle betwenn lines shoulder-elbow , elbow-wrist
        if self.is_point_in_keypoints("left_shoulder") and \
            self.is_point_in_keypoints("left_elbow") and \
                self.is_point_in_keypoints("left_wrist"):
                    ang1 = self.two_line_angle("left_shoulder", "left_elbow", "left_wrist")
        ...
        #Calculate angle between shoulder-hip, hip-ankle
        if self.is_point_in_keypoints("left_shoulder") and \
            self.is_point_in_keypoints("left_hip") and \
                self.is_point_in_keypoints("left_ankle"):
        ...
        #Calculate angle 0f line elbow-wrist
        left_elbow_wrist = self.is_point_in_keypoints("left_elbow") and self.is_point_in_keypoints("left_wrist") 
        right_elbow_wrist = self.is_point_in_keypoints("right_elbow") and self.is_point_in_keypoints("right_wrist") 
        if left_elbow_wrist or right_elbow_wrist:
            elbow = "left_elbow" if left_elbow_wrist else "right_elbow"
    ...
    def measure(self) -> None:
        """Measure planks (base func)"""
        if self.video_reader.is_opened() is False:
            print("Error File Not Found")
        
        time_adjustment = 6 / self.video_fps #6 magic number figure out wtf that is
        progress_counter = 0
        progress_bar_color = (255,255,255)
        out = cv2.VideoWriter("plank_test_output.avi", self.fourcc, self.video_fps,(self.width, self.height))
        while self.video_reader.is_opened():
            image = self.video_reader.read_frame()
            if image is None:
                print("Ignoring empty frame")
            ...
            if results.pose_landmarks is not None:
                ...
                h_m_s_ms_time = self.timer.convert_time(time) # Convert seconds into readable
                image = self.draw.pose_text(image, "Plank Timer: " + str(h_m_s_ms_time)[:10])
                progress_counter = int(int(time) % self.video_fps)
                        
            out.write(image)
            cv2.namedWindow("Plank", cv2.WND_PROP_FULLSCREEN)
            cv2.setWindowProperty("Plank", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_NORMAL)
...
class Squat(Pose):
    ...
    def measure(self) -> None:
        """Measure squats (base func)"""
        if self.video_reader.is_opened() is False:
            print("Error File Not Found")
            
        out = cv2.VideoWriter("squat_test_output.avi", self.fourcc, self.video_fps,(self.width, self.height))
        squats_count_prev = squats_count_current = progress_counter = 0
        progress_bar_color = (255,255,255)
        while self.video_reader.is_opened():
            image = self.video_reader.read_frame()
            if image is None:
            ...
            if results.pose_landmarks is not None:
                ...
                squats_count_prev = squats_count_current
                squats_count_current = self.squats_count
                if self.squats_count > 0 and abs(squats_count_current - squats_count_prev) ==1:
                    progress_counter += 1
                    if progress_counter == 10:
                        progress_counter = 0
                        progress_bar_color = random.choices(range(128,256), k=3)
            ...
            out.write(image)
            cv2.namedWindow('Squats', cv2.WND_PROP_FULLSCREEN)
            cv2.setWindowProperty('Squats', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_NORMAL)
...
class Jumpingjack(Pose):
    ...
    def pose_algorithm(self):
        ...
        if left_shoulder_ankle or right_shoulder_ankle:
            shoulder = "left_shoulder" if left_shoulder_ankle else "right_shoulder"
            ankle = "left_ankle" if left_shoulder_ankle else "right_shoulder"
            ang = self.one_line_angle(shoulder, ankle)
        elif left_hip_ankle or right_hip_ankle:
            hip = "left_hip" if left_hip_ankle else "right_hip"
            ankle = "left_ankle" if left_hip_ankle else "right_shoulder"
            ang = self.one_line_angle(hip, ankle)
        else:
            pass
        
        if norm_diff_y > 0 and 80 <= ang <= 180:
    ...
    def measure(self) -> None:
        """Measure Jumping jacks (base func)"""
        if self.video_reader.is_opened() is False:
            print("Error File Not Found")
            
        out = cv2.VideoWriter("jj_test_output.avi", self.fourcc, self.video_fps,(self.width, self.height))
        ...
        while self.video_reader.is_opened():
            ...
            if results.pose_landmarks is not None:
                ...
                self.pose_algorithm()
                #image = self._draw(image)
                image = self.draw.pose_text(image, "Jumping Jacks Count: " + str(self.jj_count))
                jj_count_prev = jj_count_current
                jj_count_current = self.jj_count
                if self.jj_count > 0 and abs(jj_count_current - jj_count_prev) ==1:
                    progress_counter += 1
                    if progress_counter == 10:
                        progress_counter = 0
                        progress_bar_color = random.choices(range(128,256), k=3)
            ...
            out.write(image)
            cv2.setWindowProperty("Jumping Jacks", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_NORMAL)
